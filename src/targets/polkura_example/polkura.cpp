...
Types::EventTypeId declare_measurement(Client& client, std::string name) {
  return client.declare_event_type(
    name,
...
void do_declarations(Client& client) {
  ...
  for (std::string name : {"WindSpeed",
                           ...
                           "AtmosphericPressure"}) {
    event_types.push_back(declare_measurement(client, name));
  }
