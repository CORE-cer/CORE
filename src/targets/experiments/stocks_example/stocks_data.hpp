...
CORE::Types::StreamTypeId do_declaration(Internal::OfflineSingleMediator& mediator, std::string stream_name, const std::array<TTypes, NTypes>& types,
  ...
  for (auto& type : types) {
    ...
    event_types.push_back(mediator.add_event_type(type, std::move(attributes_info)));
  }
  return mediator.add_stream_type(stream_name, std::move(event_types));
}
