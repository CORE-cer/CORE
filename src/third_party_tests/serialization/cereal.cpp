...
TEST_CASE("Serialization works for an object with an abstract class ptr",
  ...
  std::string serialized_object =
    CerealSerializer<ClassWithAbstractClassSharedPtr>::serialize(
      initial_object);

  // Deserialize
  ClassWithAbstractClassSharedPtr final_object =
    CerealSerializer<ClassWithAbstractClassSharedPtr>::deserialize(
      serialized_object);

  REQUIRE(initial_object.objects.size() == final_object.objects.size());
  // Detailed checks

  auto final_concrete1 = std::dynamic_pointer_cast<ConcreteClass1>(final_object.objects[0]);
  REQUIRE(final_concrete1);
  REQUIRE(obj1->values == final_concrete1->values);

  auto final_concrete2 = std::dynamic_pointer_cast<ConcreteClass2>(final_object.objects[1]);
