...
#include "core_server/internal/ceql/cel_formula/formula/unless_filter_formula.hpp"
#include "core_server/internal/ceql/cel_formula/formula/unless_formula.hpp"
...
Where parse_where(std::string query) {
  ...
  Types::StreamInfo stream_info = catalog.add_stream_type(
    {"S", {{"H", {}}, {"T", {{"a", Types::ValueTypes::INT64}}}}});
...
TEST_CASE("event unless filter", "[UNLESS]") {
  auto query = create_where_query("H UNLESS T[a == 1]");
  auto expected_formula = make_unique<UnlessFilterFormula>(
    make_unique<EventTypeFormula>("H"),
    make_unique<AtomicFilter>(
      "T",
      make_unique<InequalityPredicate>(make_unique<Attribute>("a"),
