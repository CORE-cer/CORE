...
static_assert(
  std::is_same_v<
    std::remove_cvref_t<
      decltype(std::declval<CORE::Internal::CEA::Literal<std::string_view>&>().val)>,
    std::string>);
...
TEST_CASE("String-backed predicates expose owned string constants", "[Predicate]") {
  CEA::CompareWithConstant<CEA::ComparisonType::EQUALS, std::string_view> predicate(0,
                                                                                    0,
                                                                                    "need"
                                                                                    "le");
  REQUIRE(predicate.to_string() == "Event[0] == needle");
}
}  // namespace CORE::Internal::CEA::UnitTests
