...
namespace DatabaseManager {
    ...
    public:
        ...
        void add_event_type(std::string&& event_name, std::vector<AttributeInfo>&& event_attributes, uint64_t event_type_id);

        void add_stream_type(std::string stream_name, std::vector<uint64_t>&& stream_event_types, uint64_t stream_type_id);
            
        void connect_database() {
            try {
                pqxx::connection conn("dbname=core user=postgres password=postgres host=localhost port=5432");
                if (conn.is_open()) {
    ...
    private:
        void create_table() {
            try {
                ...
                std::string eventTableSQL = "CREATE TABLE IF NOT EXISTS events("
                                            "event_id SERIAL PRIMARY KEY,"
                                            "event_name VARCHAR(255),"
                                            "event_attributes JSONB"
                                            ");";

                std::string streamTableSQL = "CREATE TABLE IF NOT EXISTS streams ("
