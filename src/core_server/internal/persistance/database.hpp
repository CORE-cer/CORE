...
class Database {
 ...
 public:
  ...
  void add_stream_type(std::string stream_name,
    ...
    query += "event_type_id INT, ";
    query += "timestamp INT";
    query += ", FOREIGN KEY (event_type_id) REFERENCES eventnames(event_type_id)"; //FK
    query += ");";
...
void create_streams_table(const std::string& db_connection_query, const std::vector<std::string>& event_tables) {
    std::string query = "CREATE TABLE streams (stream_uid INT, event_type_id INT, PRIMARY KEY (stream_uid, event_type_id)";

    query += ", FOREIGN KEY (stream_uid) REFERENCES streamnames(stream_uid)"; //FK
    query += ");";
  ...
  void create_event_names_table(const std::string &db_connection_query) {
      std::string query = "CREATE TABLE eventnames (event_type_id INT PRIMARY KEY, event_table_name VARCHAR(255));";
      execute_query(query,db_connection_query);
  }

  void create_streams_names_table(const std::string &db_connection_query) {
      std::string query = "CREATE TABLE streamnames (stream_uid INT PRIMARY KEY, stream_table_name VARCHAR(255));";
      execute_query(query,db_connection_query);
  }
  ...
  void execute_query(const std::string& query, const std::string& connection_query) {
  try {
