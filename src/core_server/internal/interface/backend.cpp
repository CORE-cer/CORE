...
Backend::Backend(std::atomic<Types::PortNumber>& next_available_port)
...
Types::EventTypeId
Backend::add_event_type(std::string event_name,
...
Types::EventInfo Backend::get_event_info(Types::EventTypeId event_type_id) {
  return catalog.get_event_info(event_type_id);
}

Types::EventInfo Backend::get_event_info(std::string event_name) {
  return catalog.get_event_info(event_name);
}

std::vector<Types::EventInfo> Backend::get_all_events_info() {
  return catalog.get_all_events_info();
}

// TODO: Add error to catalog add stream type and propogate to ClientMessageHandler
Types::StreamTypeId
Backend::add_stream_type(std::string stream_name,
...
Types::StreamInfo Backend::get_stream_info(Types::StreamTypeId event_type_id) {
  return catalog.get_stream_info(event_type_id);
}

Types::StreamInfo Backend::get_stream_info(std::string event_name) {
  return catalog.get_stream_info(event_name);
}

std::vector<Types::StreamInfo> Backend::get_all_streams_info() {
  return catalog.get_all_streams_info();
}

// TODO: Propogate parse error to ClientMessageHandler
void Backend::declare_query(std::string query,
                   std::function<void(Types::Enumerator)> result_handler) {
  Internal::CEQL::Query parsed_query = Internal::Parsing::Parser::parse_query(
    query);
...
void Backend::update_space_of_ring_tuple_queue() {
...
void Backend::write_int(std::shared_ptr<Types::Value>& attr) {
...
void Backend::write_double(std::shared_ptr<Types::Value>& attr) {
...
void Backend::write_bool(std::shared_ptr<Types::Value>& attr) {
...
void Backend::write_string_view(std::shared_ptr<Types::Value>& attr) {
...
void Backend::write_date(std::shared_ptr<Types::Value>& attr) {
