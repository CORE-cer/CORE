...
class Attribute : public MathExpr<GlobalType> {
 public:
  ...
  // The position and stored type of a strongly typed attribute are known when the
  // query is compiled, so the pairing (GlobalType, LocalType) decides the formula
  // directly. See attribute_formula for the table and what stays opaque.
  std::optional<FormulaBuilder::Handle>
  translate(FormulaBuilder& builder, Types::UniqueEventTypeId event_type) const override {
    return attribute_formula<GlobalType, LocalType>(builder, event_type, pos);
  }
