...
class Interleaved_Conjunction : public LogicalCEATransformer<Interleaved_Conjunction> {
 public:
  ...
  LogicalCEA eval(LogicalCEA& left, LogicalCEA& right) {
    std::cout << "Printing info about logical CEA's " << std::endl;
    ...
    /** Step 2:
     * Aφ is now again a form of product automaton between Aψ1 and Aψ2. 
     * To obtain the semantics of ALL this product automaton will allow a sub-automaton 
     * (i.e., Aψ1 or Aψ2) to start later or stop earlier than the other sub-automaton in the run.
     * 
     * Formally, Aφ = (Qψ1 × Qψ2, Δφ, Iψ1 × Iψ2, Fψ1 × Fψ2) where Δφ is defined as follows:
     * Δφ = {((q1, q2), P1 ∩ P2, L1 ∪ L2, (p1, p2)) | (q1, P1, L1, p1) ∈ Δψ1, (q2, P2, L2, p2) ∈ Δψ2}
            ...
            for (size_t j = 0; j < right.transitions.size(); ++j) {
                for (const auto& transition2 : right.transitions[j]) {
                    ...
                    if (intersection.type != PredicateSet::Contradiction) {
                        ...
                        out.transitions[source].push_back(std::make_tuple(intersection, combined_mark, target));
                    }
                }
            }
        }
    ...
    for (uint64_t state_left : initial_states_left) {
        for (size_t j = 0; j < right.amount_of_states; ++j) {
            for (const auto& transition2 : right.transitions[j]) {
                ...
                // Compute the target state
                EndNodeId target = state_left * right.amount_of_states + std::get<2>(transition2);
  ...
  void handle_initial_state(LogicalCEA& cea) {
    ...
    std::cout << "cea after making new initial state: " << std::endl;
    std::cout << " " << cea.to_string() << std::endl;
  }
  ...
  void handle_final_state(LogicalCEA& cea) {
    ...
    std::cout << "cea after making new final state: " << std::endl;
    std::cout << " " << cea.to_string() << std::endl;
  }
