...
std::string ClientMessageHandler::operator()(
  ...
  return CerealSerializer<ServerResponse>::serialize(handle_client_request(
    CerealSerializer<ClientRequest>::deserialize(serialized_client_request)));
}

ServerResponse
ClientMessageHandler::handle_client_request(ClientRequest request) {
...
ClientMessageHandler::event_declaration(std::string s_event_info) {
  auto event_info = CerealSerializer<
    std::pair<std::string,
              std::vector<AttributeInfo>>>::deserialize(s_event_info);
  ...
  EventTypeId id = catalog.add_event_type(std::move(name),
...
ServerResponse ClientMessageHandler::list_all_events() {
  ...
  return ServerResponse(CerealSerializer<std::vector<EventInfo>>::serialize(
                          info),
...
ClientMessageHandler::stream_declaration(std::string s_stream_info) {
  auto stream_info = CerealSerializer<
    std::pair<std::string, std::vector<EventTypeId>>>::deserialize(s_stream_info);
  ...
  StreamTypeId id = catalog.add_stream_type(std::move(name),
...
ClientMessageHandler::stream_info_from_id(std::string s_stream_id) {
  auto stream_id = CerealSerializer<StreamTypeId>::deserialize(s_stream_id);
...
ServerResponse ClientMessageHandler::list_all_streams() {
  ...
  return ServerResponse(CerealSerializer<std::vector<StreamInfo>>::serialize(
                          info),
                        ServerResponseType::StreamInfoVector);
}

ServerResponse ClientMessageHandler::add_query(std::string s_query_info) {
  // TODO: Change this to a CEA. Right now it's going to be a string
  // that does nothing.
