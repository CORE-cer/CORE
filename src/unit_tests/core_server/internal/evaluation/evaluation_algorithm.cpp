...
TEST_CASE("Evaluation on the example stream of the paper.") {
  ...
  REQUIRE(outputs.size() == evaluator.n_res(0));
  ...
  INFO(output_to_string(next_output_enumerator));
  REQUIRE(outputs.size() == 0);
  REQUIRE(outputs.size() == evaluator.n_res(1));
  ...
  INFO(output_to_string(next_output_enumerator));
  REQUIRE(outputs.size() == 0);
  REQUIRE(outputs.size() == evaluator.n_res(2));
  ...
  INFO(output_to_string(next_output_enumerator));
  REQUIRE(outputs.size() == 0);
  REQUIRE(outputs.size() == evaluator.n_res(3));
  ...
  REQUIRE(outputs.size() == evaluator.n_res(4));
  for (std::pair<std::pair<uint64_t, uint64_t>,
  ...
  INFO(output_to_string(next_output_enumerator));
  REQUIRE(outputs.size() == 0);
  REQUIRE(outputs.size() == evaluator.n_res(6));
...
TEST_CASE(
  ...
  INFO(output_to_string(next_output_enumerator));
  REQUIRE(outputs.size() == 0);
  REQUIRE(outputs.size() == evaluator.n_res(5));
...
TEST_CASE("Evaluation of a query with contiguous events") {
  ...
  REQUIRE(outputs.size() == evaluator.n_res(7));
  for (std::pair<std::pair<uint64_t, uint64_t>,
                 std::vector<RingTupleQueue::Tuple>> output : outputs) {
  }
  ...
  INFO(output_to_string(next_output_enumerator));
  REQUIRE(outputs.size() == 0);
  REQUIRE(outputs.size() == evaluator.n_res(8));
