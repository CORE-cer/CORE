...
std::string send(PortNumber inner_thread_port_number,
                 Event& event,
...
TEST_CASE("A sent event is read back by dummy EventStreamer",
  ...
  QueryEvaluator streamer(5001, 5555, queue);
  streamer.start();
  ...
  std::string message = send(5001,
                             event,
                             streamer.get_inner_thread_context());
