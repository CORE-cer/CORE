...
TEST_CASE("Connect to database", "[server coordination]") {
  ...
  // std::vector<CORETypes::Value> eventAttributes;
  // eventAttributes.push_back(CORETypes::Value("Attribute1", std::to_string(123)));
  // eventAttributes.push_back(CORETypes::Value("Attribute2", std::to_string(3.14)));

  // uint64_t event_type_id = 1; 
  // uint64_t event_uid = 1;  
  // uint64_t stream_uid = 1;  
  // database.add_event(event_type_id, event_uid, stream_uid, eventTimestamp, eventAttributes);
}
